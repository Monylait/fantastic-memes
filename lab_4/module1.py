...
def S(mes):
    ...
    for i in mes:
        try:
            print(i)
            n=int(i[7])*1+int(i[6])*2+int(i[5])*4+int(i[4])*8+int(i[3])*16+int(i[2])*32+int(i[1])*64+int(i[0])*128
            print(n)
            mes_chisla.append(n)
        except:
            continue
    ...
    for i in range(0,64):
       result[i] = pi[mes_chisla[i]];
    return result


def LPS(mes):

    result=[mes[i:i+8] for i in range(0,len(mes),8)]
    return L(P(S(result)))
...
M_byte=list_variables[1].encode("utf-8")
...
def S(mes): # S-преобразование (По Пи)
    result = [0] * 64
    for i in range(0, 64):
        result[i] = Pi[mes[i]]
    return result
...
def P(mes): # P-преобразование (По Тау)
    result = [0] * 64
    for i in range(0, 64):
        result[i] = mes[tau[i]]
    return result
...
def L(mes): # L-преобразование по матрице А, разбитой по 8 байт
    l = (
    ...
    result = [0] * 64
    i = 7 # Блок байт (8 штук, от 0 до 7)
    while (i >= 0):
       n = 0  # Столбец (от 0 до 63)
       while (n <= 7):
           p = 63  # Строка
           j = 7
           while (j >= 0):
               k = 0 # Что-то по типу сдвига
               while (k <= 7):
                   if ((mes[i * 8 + j] >> k) & 1):
...
def E(K, m): # E - преобразование через C1-C12, Ci заранее переведены в байты
    C = (
...
def g(N, h, m): # Функция сжатия - проводим преобразования через формулы.
    var = XoR(h, N)
    var = LPS(var)
    var = E(var, m)
    var = XoR(var, h)
...
mes=""
while flag:
    print("Stribog. Step 1\n")
    print(">> Give message - 1\n")
    print(">> Go to step 2 - 2\n")
    print(">> Exit - 3\n")
    point = str(input("\n>>> "))
    if point == "1":
        while True:
